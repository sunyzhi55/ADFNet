...
# ── eval_mode 常量 ──────────────────────────────────────────────
EVAL_FATIGUE = "fatigue"                    # FG train → FG test
EVAL_FATIGUE_TO_GAIPAT = "fatigue_to_gaipat"  # FG train → GAIPAT test
EVAL_GAIPAT = "gaipat"                      # GAIPAT train → GAIPAT test
EVAL_GAIPAT_TO_FATIGUE = "gaipat_to_fatigue"  # GAIPAT train → FG test
...
def main() -> None:
    ...
    cfg["exp_name"] = f"{cfg['exp_name']}_loso_{eval_mode}"
    ...
    save_hparams(
        ...
        script="run_loso.py", task_mode=task_mode, timestamp=timestamp,
        extra={
            "eval_mode": eval_mode,
    ...
    for fold in folds:
        ...
        if not skip_training:
            ...
            logger.info("%s: train=%d, val=%d", fold.name, len(train_dataset), len(val_dataset))
    ...
    if is_cross_dataset:
        ...
        if cross_rows:
            ...
            cross_output = Path(cfg["training"]["output_dir"]) / f"loso_cross_to_{target_name}_{task_mode}.csv"
